...
def validate(audio, model, embedder, testloader, writer, step):
    ...
    with torch.no_grad():
        for batch in testloader:
            dvec_mel, target_wav, mixed_wav, new_target_wav, target_mag, new_target_mag, mixed_mag, mixed_phase = batch[0]

            dvec_mel = dvec_mel.cuda()
            target_mag = target_mag.unsqueeze(0).cuda()
            mixed_mag = mixed_mag.unsqueeze(0).cuda()
            new_target_mag = new_target_mag.unsqueeze(0).cuda()
            ...
            new_target_mag = new_target_mag[0].cpu().detach().numpy()
            ...
            writer.log_evaluation(test_loss, sdr,
                                  ...
                                  target_wav, new_target_wav,
                                  mixed_mag.T, target_mag.T, new_target_mag.T, est_purified_mag.T, est_noise_mag.T,
                                  step)
            break
